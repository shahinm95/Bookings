...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	err := m.DB.QueryRowContext(ctx, query,
		res.FirstName, res.LastName, res.Email, res.Phone, res.StartDate, res.EndDate, res.RoomID, time.Now(),
		time.Now(),
	).Scan(&newID)
	if err != nil {
		return 0, err
...
// Authenticate athenticates a user
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&i.ID, &i.FirstName, &i.LastName, &i.Email, &i.Phone, &i.StartDate, &i.EndDate, &i.RoomID, &i.CreatedAt,
			&i.UpdatedAt, &i.Processed, &i.Room.ID, &i.Room.RoomName,
		)
		if err != nil {
			return reservations, err
		}
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&i.ID, &i.FirstName, &i.LastName, &i.Email, &i.Phone, &i.StartDate, &i.EndDate, &i.RoomID, &i.CreatedAt,
			&i.UpdatedAt, &i.Room.ID, &i.Room.RoomName,
		)
		if err != nil {
			return reservations, err
		}
...
// GetReservationById return a reservation by given ID
func (m *postgresDBRepo) GetReservationById(id int) (models.Reservation, error) {
	...
	row := m.DB.QueryRowContext(ctx, query, id)
	err := row.Scan(&res.ID, &res.FirstName, &res.LastName, &res.Email, &res.Phone, &res.StartDate,
		&res.EndDate, &res.RoomID, &res.CreatedAt, &res.UpdatedAt, &res.Processed, &res.Room.ID,
		&res.Room.RoomName,
	)
	if err != nil {
		return res, err
...
//DeletBlockForRoom delete a room restriction
func (m *postgresDBRepo) DeletBlockByID(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
